...
pub(crate) struct DepReport {
    ...
    /// PR assignees from the effective packageRule or repo config.
    #[serde(skip_serializing_if = "Vec::is_empty", default)]
    ...
    /// PR creation priority from the effective packageRule.
    /// Higher values → created first; negative → created last.
    /// `None` = not configured (treat as 0).
    #[serde(rename = "prPriority", skip_serializing_if = "Option::is_none")]
    ...
    /// ISO 8601 publish timestamp of the proposed latest version, when available
    ...
    /// Dependency type string (e.g. `"dependencies"`, `"devDependencies"`, `"dev"`).
    /// Populated for managers that distinguish dep types (npm, cargo).  `None` for
    /// managers where all deps are equivalent.
    ///
