//! Per-manager pipeline blocks for `process_repo`.
//!
//! All manager processing blocks were a single 8,400-line function in
//! `main.rs`. This module extracts them into [`process_all_managers`] which
//! takes a [`crate::context::RepoPipelineCtx`] holding shared state.
//!
//! Each `// ── Manager ──` comment block is preserved verbatim so future
//! splits into per-manager sub-modules remain straightforward.
...
use crate::context::RepoPipelineCtx;
use crate::output;
...
///
/// Binds immutable shared fields as short locals so the manager blocks
/// look nearly identical to their original form inside `process_repo`.
