...
impl Counts {
    fn of(rows: &[&Row]) -> Self {
        Self {
            full: count(rows, "full"),
            partial: count(rows, "partial"),
            stub: count(rows, "stub"),
            pending: count(rows, "pending"),
            oos: count(rows, "out-of-scope"),
            optout: count(rows, "opt-out"),
        }
    }
    /// Files with some Rust implementation.
    fn mapped(&self) -> usize {
        self.full + self.partial + self.stub
    }
    /// Files intentionally not ported (out-of-scope tag + opt-out registry).
    fn excluded(&self) -> usize {
        self.oos + self.optout
    }
    /// Trailing status cells `full | partial | stub | pending | oos | opt-out`.
    fn cells(&self) -> String {
        format!(
            "{} | {} | {} | {} | {} | {}",
            self.full, self.partial, self.stub, self.pending, self.oos, self.optout
        )
    }
}
...
pub(crate) fn write_pages(
    ...
    let mut rows = reconcile(upstream, tags);

    // Apply the opt-out registry: a pending file with a decision becomes
    // `opt-out` (excluded, with the reason) so it is no longer picked.
    for r in &mut rows {
        if r.status == "pending"
            && let Some(reason) = optout.source_reason(&r.ts)
        {
            r.status = "opt-out".to_string();
            if r.note.is_empty() {
                r.note = reason.to_string();
            }
        }
    }
...
fn render_readme(
    ...
    let c = Counts::of(&refs);
    let total = rows.len();
    let in_scope = total - c.excluded();

    let mut out = String::new();
    out.push_str("# Source Mapping (impl files)\n\n");
    out.push_str("Auto-generated by `cargo run -p parity-cli -- source`. **Do not hand-edit.**\n");
    out.push_str("Status lives in `@parity` tags in the Rust source; exclusions in ");
    out.push_str("`docs/parity/opt-out.toml`. Click a group to see its modules and mapping.\n\n");
    out.push_str(
        "Status: `full` · `partial` · `stub` · `pending` (no tag) · `out-of-scope` (tag) · \
         `opt-out` (registry — will never be ported, with a reason).\n\n",
    );
    out.push_str(&format!(
        "**Coverage:** {}/{in_scope} in-scope files mapped \
         (full={} partial={} stub={} pending={} out-of-scope={} opt-out={}). \
    ...
    out.push_str("| Group | Files | full | partial | stub | pending | oos | opt-out |\n");
    out.push_str("|---|--:|--:|--:|--:|--:|--:|--:|\n");
    for (group, modules) in by_group {
    ...
    out.push_str("| Module | Files | full | partial | stub | pending | oos | opt-out |\n");
    out.push_str("|---|--:|--:|--:|--:|--:|--:|--:|\n");
...
fn render_group_page(
    ...
    let grp_rows: Vec<&Row> = modules
        ...
        .flat_map(|m| by_module[*m].iter().copied())
        .collect();
    let c = Counts::of(&grp_rows);
    let in_scope = grp_rows.len() - c.excluded();
