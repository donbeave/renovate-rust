...
pub(crate) fn render_report(specs: &[SpecFile], ported: &[Ported]) -> String {
    let Analysis { stats, deleted } = analyze(specs, ported);

    let total_it: usize = specs.iter().map(|s| s.it_count).sum();
    let total_ported: usize = stats.iter().map(|s| s.ported).sum();
    let total_pending = total_it - total_ported;

    let mut out = String::new();
    out.push_str("# Renovate Test-Map (it/test parity)\n\n");
    out.push_str("Auto-generated by `cargo run -p parity-cli -- test`.\n");
    out.push_str("**Do not hand-edit.** Recomputed from upstream `*.spec.ts` files (whole repo) ");
    out.push_str("and `// Ported:` comments in the Rust tests.\n\n");
    out.push_str(
        "Each upstream `it()`/`test()` is `ported` or `pending`. A Rust `// Ported:` whose \
         upstream identity is gone is `deleted` (kept for review, never auto-removed). \
         Renames upstream show as an old `deleted` + a new `pending`.\n\n",
    );
    out.push_str(&format!(
        "**Coverage:** {total_ported}/{total_it} upstream tests ported ({total_pending} pending) \
         across {} spec files. **Deleted (review):** {}.\n\n",
        specs.len(),
        deleted.len(),
    ));
    ...
    if !deleted.is_empty() {
        ...
        out.push_str("Rust tests whose upstream identity no longer exists. Kept in the Rust ");
        out.push_str("repo; review whether to remove or re-point.\n\n");
