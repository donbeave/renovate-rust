...
pub fn validate_decrypted_value_with_endpoint(
    ...
    let azcol = azure_collection.as_deref().map(|c| ensure_trailing_slash(&c.to_uppercase()));
    ...
    if !repo.is_empty() {
        ...
        for rp in &repositories {
            if org_prefixes.iter().any(|prefix| {
                rp.starts_with(prefix.as_str())
                    && azcol.as_deref() != Some(prefix.as_str())
            }) {
                return Some(value);
            }
        }
...
fn validate_decrypted_value_azure_dev() {
    ...
    let cases: &[(&str, &str, Option<&str>)] = &[
        ...
        (r#"{"o":"fgh","r":"rp1","v":"zv-1"}"#, "fgh/rp1", Some("zv-1")),
        (r#"{"o":"fgh","r":"","v":"zv-2"}"#, "fgh/rp1", Some("zv-2")),
        (r#"{"o":"az123/fgh","r":"rp1","v":"zv-3"}"#, "fgh/rp1", Some("zv-3")),
        (r#"{"o":"az123/fgh","r":"","v":"zv-4"}"#, "fgh/rp1", Some("zv-4")),
        (r#"{"o":"az123/*","r":"","v":"zv-5"}"#, "fgh/rp1", Some("zv-5")),
        ...
        (r#"{"o":"az123/*,any/*","r":"","v":"zv-12"}"#, "fgh/rp1", Some("zv-12")),
        (r#"{"o":"fgh,any/*","r":"","v":"zv-13"}"#, "fgh/rp1", Some("zv-13")),
        (r#"{"o":"az123/,any/*","r":"","v":"zv-14"}"#, "fgh/rp1", None),
        (r#"{"o":"any/*,fgh/","r":"","v":"zv-15"}"#, "fgh/rp1", Some("zv-15")),
    ...
    for &(str_, repo, expected) in cases {
        ...
        assert_eq!(result.as_deref(), expected, "validate_azure_dev({str_:?}, {repo:?})");
    }
}
...
fn validate_decrypted_value_azure_tfs() {
    ...
    for &(str_, repo, expected) in cases {
        let result = validate_decrypted_value_with_endpoint(str_, repo, Some(endpoint));
        assert_eq!(result.as_deref(), expected, "validate_azure_tfs({str_:?}, {repo:?})");
    }
}
...
fn validate_decrypted_value_azure_invalid_endpoint() {
    ...
    assert_eq!(validate_decrypted_value_with_endpoint(r#"{"o":"proj","r":"repo","v":"any-1"}"#, "proj/repo", Some(endpoint)).as_deref(), Some("any-1"));
    assert_eq!(validate_decrypted_value_with_endpoint(r#"{"o":"proj","r":"","v":"any-2"}"#, "proj/repo", Some(endpoint)).as_deref(), Some("any-2"));
    assert_eq!(validate_decrypted_value_with_endpoint(r#"{"o":"col/proj","r":"","v":"any"}"#, "proj/repo", Some(endpoint)), None);
    assert_eq!(validate_decrypted_value_with_endpoint(r#"{"o":"col/*","r":"","v":"any"}"#, "proj/repo", Some(endpoint)), None);
}
...
fn validate_decrypted_value_azure_no_collection() {
    ...
    assert_eq!(validate_decrypted_value_with_endpoint(r#"{"o":"proj","r":"repo","v":"any-3"}"#, "proj/repo", Some(endpoint)).as_deref(), Some("any-3"));
    assert_eq!(validate_decrypted_value_with_endpoint(r#"{"o":"proj","r":"","v":"any-4"}"#, "proj/repo", Some(endpoint)).as_deref(), Some("any-4"));
    assert_eq!(validate_decrypted_value_with_endpoint(r#"{"o":"col/proj","r":"","v":"any"}"#, "proj/repo", Some(endpoint)), None);
    assert_eq!(validate_decrypted_value_with_endpoint(r#"{"o":"col/*","r":"","v":"any"}"#, "proj/repo", Some(endpoint)), None);
}
...
fn get_azure_collection_pathname_no_slash() {
    assert_eq!(get_azure_collection("https://dev.azure.com/aaa").as_deref(), Some("aaa"));
}
...
fn get_azure_collection_pathname_2_levels_no_slash() {
    assert_eq!(get_azure_collection("https://dev.azure.com/aaa/bbb").as_deref(), Some("aaa/bbb"));
}
