...
"#;
    ...
    // Ported: "extracts the current version from a valid wrapper script" — lib/modules/manager/batect-wrapper/extract.spec.ts line 17
    #[test]
    fn extracts_version() {
    ...
    // Ported: "returns null for file without version information" — lib/modules/manager/batect-wrapper/extract.spec.ts line 13
    #[test]
    fn no_version_line_returns_none() {
        assert!(extract("#!/bin/bash\necho hello\n").is_none());
    }

    // Ported: "returns null for empty wrapper file" — lib/modules/manager/batect-wrapper/extract.spec.ts line 9
    #[test]
    fn empty_returns_none() {
        assert!(extract("").is_none());
    }

    // Ported: "returns the first version from a wrapper script with multiple versions" — lib/modules/manager/batect-wrapper/extract.spec.ts line 31
    #[test]
