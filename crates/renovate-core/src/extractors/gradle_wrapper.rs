...
/// Parse `toolchainVersion` from a `gradle-daemon-jvm.properties` file.
///
/// Returns the version string (e.g. `"21"`) or `None` when the property
/// is absent.
///
/// Mirrors the parsing part of `getJvmConfiguration` from
/// `lib/modules/manager/gradle-wrapper/utils.ts`.
pub fn parse_jvm_toolchain_version(content: &str) -> Option<String> {
    for line in content.lines() {
        let trimmed = line.trim();
        if let Some(val) = trimmed.strip_prefix("toolchainVersion=") {
            let v = val.trim();
            if !v.is_empty() {
...
/// Parse a `JavaLanguageVersion.of(N)` call from a Gradle build file.
///
/// Returns the version string (e.g. `"21"`) or `None` when the pattern is
/// absent.
///
/// Mirrors the parsing part of `getJavaLanguageVersion` from
/// `lib/modules/manager/gradle-wrapper/utils.ts`.
...
/// Format a toolchain or language version number into a semver constraint.
///
/// Mirrors the constraint format produced by `getJavaConstraint` when it
/// finds a toolchain version or language version in a Gradle build file:
/// `toolchainVersion=N` or `JavaLanguageVersion.of(N)` → `^N.0.0`.
pub fn java_constraint_from_version(version: &str) -> String {
    format!("^{}.0.0", version)
}
...
";
    ...
    fn parse_java_language_version_returns_none_for_no_pattern() {
        let content =
            r#"dependencies { implementation "com.google.protobuf:protobuf-java:2.17.0" }"#;
    ...
    fn toolchain_version_constraint_from_daemon_jvm_content() {
        ...
        let version = parse_jvm_toolchain_version(content).unwrap();
    ...
    fn language_version_constraint_from_build_gradle_content() {
        ...
        let version = parse_java_language_version(content).unwrap();
