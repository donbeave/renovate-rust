...
**Total tests:** 19 | **Ported:** 12 | **Actionable:** 19 | **Status:** partial
...
| should parse content with single document with schema | 26 | not-applicable | — | — | Uses Zod schema validation; Rust uses serde, no Zod equivalent |
| should parse content with multiple documents | 50 | ported | `util.rs` | `test_parse_yaml_multiple` | — |
| should parse content with multiple documents with schema | 70 | not-applicable | — | — | Uses Zod schema validation |
| should throw if schema does not match | 102 | not-applicable | — | — | Uses Zod schema validation |
| should throw if schema does not match and failureBehaviour "throw" | 122 | not-applicable | — | — | Uses Zod schema validation |
| should still return valid elements if schema does not match with "filter" behaviour | 143 | not-applicable | — | — | Uses Zod schema with filter behavior |
...
| should parse content with single document with schema | 253 | not-applicable | — | — | Uses Zod schema validation |
| should throw with single document with schema if parsing fails | 275 | not-applicable | — | — | Uses Zod schema validation |
