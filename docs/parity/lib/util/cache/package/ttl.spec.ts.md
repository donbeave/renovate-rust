...
| returns undefined when exact match is not a number | 45 | not-applicable | — | — | TypeScript type-system test; uses @ts-expect-error to pass non-numeric string; Rust HashMap<String, i64> enforces numeric type at deserialization |
...
| skips non-numeric values and selects next longest matching pattern | 228 | not-applicable | — | — | TypeScript type-system test; uses @ts-expect-error to inject 'invalid' string; Rust type system prevents non-numeric TTL values |
...
| treats null and undefined values as invalid | 286 | not-applicable | — | — | TypeScript type-system test; uses @ts-expect-error to pass null/undefined; Rust type system prevents null TTL values |
...
| treats string numbers as invalid, only accepts number types | 330 | not-applicable | — | — | TypeScript type-system test; uses @ts-expect-error to pass string '90'; Rust i64 enforces numeric type at deserialization |
...
| uses fallback when override is not a number | 477 | not-applicable | — | — | TypeScript type-system test; uses @ts-expect-error to pass non-numeric string; Rust type system prevents this case |
