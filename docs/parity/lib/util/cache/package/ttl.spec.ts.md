...
| returns undefined when exact match is not a number | 45 | not-applicable | — | — | TypeScript type-system test; uses `@ts-expect-error` to force a string into a number-typed field — impossible in Rust's type system |
...
| skips non-numeric values and selects next longest matching pattern | 228 | not-applicable | — | — | TypeScript type-system test; uses `@ts-expect-error` to force a string into a number-typed field — impossible in Rust's type system |
...
| treats null and undefined values as invalid | 286 | not-applicable | — | — | TypeScript type-system test; uses `@ts-expect-error` to force null/undefined into a number-typed field — impossible in Rust's type system |
| handles very large numbers | 306 | ported | `cache/package.rs` | `get_ttl_override_handles_very_large_numbers` | — |
| handles negative numbers | 318 | ported | `cache/package.rs` | `get_ttl_override_handles_negative_values` | — |
| treats string numbers as invalid, only accepts number types | 330 | not-applicable | — | — | TypeScript type-system test; uses `@ts-expect-error` to force a string into a number-typed field — impossible in Rust's type system |
...
| uses fallback when override is not a number | 477 | not-applicable | — | — | TypeScript type-system test; uses `@ts-expect-error` to force a string into a number-typed field — impossible in Rust's type system |
