# Renovate Test Detail

[Back to test map](../../../renovate-test-map.md)

## `lib/modules/manager/metadata.spec.ts`

**Reference:** https://github.com/renovatebot/renovate/blob/main/lib/modules/manager/metadata.spec.ts
**Total tests:** 1 | **Ported:** 0 | **Actionable:** 1 | **Status:** done

### `modules/manager/metadata`

| Original test name | Line | Status | Rust file | Rust test name | Reason |
|---|---|---|---|---|---|
| %s has readme with no h1 or h2 | 22 | not-applicable | — | — | TypeScript module system test; iterates manager directories via fs.readdirSync and reads readme.md files — same pattern as versioning-metadata |

---

