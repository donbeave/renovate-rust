# Renovate Test Detail

[Back to test map](../../../../renovate-test-map.md)

## `lib/modules/manager/gomod/integration.spec.ts`

**Reference:** https://github.com/renovatebot/renovate/blob/main/lib/modules/manager/gomod/integration.spec.ts
**Total tests:** 1 | **Ported:** 0 | **Actionable:** 0 | **Status:** done

### `when constraintsFiltering=strict`

| Original test name | Line | Status | Rust file | Rust test name | Reason |
|---|---|---|---|---|---|
| only suggests updates within the minor version of the `go` directive | 21 | not-applicable | — | — | mocking framework internals — tests httpMock integration for end-to-end Go directive constraint filtering |

---

