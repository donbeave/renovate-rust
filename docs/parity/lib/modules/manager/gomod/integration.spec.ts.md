...
|---|---|---|---|---|---|
| only suggests updates within the minor version of the `go` directive | 21 | not-applicable | — | — | integration test with complex HTTP mocking — tests end-to-end Go directive constraint filtering with httpMock; Rust architecture differs significantly (HTTP layer, extraction, versioning separation); individual components tested elsewhere |
