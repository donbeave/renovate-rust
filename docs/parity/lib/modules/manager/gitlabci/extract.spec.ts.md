...
| extracts multiple included image lines | 46 | not-applicable | Mock framework internals — tests gitlabci extract via vitest-mocked fs; Rust tests this at different layer | — | multi-file include infrastructure — tests GitLab CI include: local/project: statements to load external YAML files; Rust implementation doesn't support multi-file include resolution; individual file extraction tested elsewhere |
