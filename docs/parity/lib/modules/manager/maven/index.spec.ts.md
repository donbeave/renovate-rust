...
| should update existing dependency defined via properties | 43 | pending | — | — | `resolveParents` multi-POM inheritance not implemented in Rust |
| should not touch content if new and old versions are equal | 67 | ported | `extractors/maven.rs` | `maven_index_no_touch_when_equal` | — |
| should update to version of the latest dep in implicit group | 79 | pending | — | — | Implicit group detection via shared properties not implemented |
| should return null for ungrouped deps if content was updated outside | 135 | pending | — | — | Implicit group detection not implemented |
