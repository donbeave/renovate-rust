...
| throws on disabled host | 214 | not-applicable | Mock framework internals — tests repology via nock HTTP mocks; Rust tests this at different layer | — | hostRules integration test — tests that datasources respect hostRules.enabled=false to disable hosts; Rust datasources don't integrate with host_rules module (architectural difference); host_rules filtering would require cross-cutting implementation across all datasources and HTTP layer |
