...
| calls simpleGit with emptyEnv if no hostrules exist | 124 | not-applicable | — | — | mocking framework internals — tests TypeScript `simpleGit` mock infrastructure; git subprocess env vars are tested in `util.rs` |
| calls simpleGit with git envs if hostrules exist | 135 | not-applicable | — | — | mocking framework internals — tests TypeScript `simpleGit` mock infrastructure |
| calls simpleGit with git envs if hostrules exist for datasource type git-refs | 162 | not-applicable | — | — | mocking framework internals — tests TypeScript `simpleGit` mock infrastructure |
