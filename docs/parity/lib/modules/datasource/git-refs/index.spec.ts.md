...
**Total tests:** 11 | **Ported:** 8 | **Actionable:** 3 | **Status:** pending
...
| calls simpleGit with emptyEnv if no hostrules exist | 124 | pending | — | — | Rust datasource receives `ls_remote` as a string; git subprocess execution and env vars are handled by the caller layer, tested in `util.rs` |
| calls simpleGit with git envs if hostrules exist | 135 | pending | — | — | Same reason as line 124 |
| calls simpleGit with git envs if hostrules exist for datasource type git-refs | 162 | pending | — | — | Same reason as line 124 |
