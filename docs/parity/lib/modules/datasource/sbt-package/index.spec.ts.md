...
| continues when parseUrl returns null for packageRootUrl | 284 | ported | `crates/renovate-core/src/datasources/sbt_package.rs` | `continues_when_package_root_url_fails` | URL parse failure simulated via HTTP 404 on artifact subdirs |
| skips pkgUrl when parseUrl returns null for it | 322 | ported | `crates/renovate-core/src/datasources/sbt_package.rs` | `skips_pkg_url_when_parse_url_returns_null` | URL parse failure simulated via partial HTTP responses |
