...
**Total tests:** 6 | **Ported:** 4 | **Actionable:** 2 | **Status:** pending
...
| should return true for different status code | 45 | pending | — | — | httpMock-based cache invalidation test — tests checkIfModified() via httpMock status code mocking; Rust Debian datasource lacks cache invalidation feature; would require full checkIfModified implementation plus HTTP status code handling |
| should return true if request failed | 60 | pending | — | — | httpMock-based cache invalidation test — tests checkIfModified() via httpMock error simulation; Rust Debian datasource lacks cache invalidation feature; would require full checkIfModified implementation plus HTTP error handling |
