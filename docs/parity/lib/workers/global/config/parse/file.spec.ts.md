...
**Total tests:** 15 | **Ported:** 8 | **Actionable:** 7 | **Status:** partial

### `workers/global/config/parse/file › .getConfig()`

| Original test name | Line | Status | Rust file | Rust test name | Reason |
|---|---|---|---|---|---|
| parses %s > %s  | 27 | pending | — | — | JS config files / env var resolution (TS-specific) |
| migrates  | 56 | pending | — | — | JS config files / env var resolution (TS-specific) |
| warns if config is invalid  | 68 | pending | — | — | JS config files / env var resolution (TS-specific) |
...
| fatal error and exit if error in parsing %s  | 84 | pending | — | — | JS config files / env var resolution (TS-specific) |
| fatal error and exit if custom config file does not exist | 112 | ported | `file.rs` | `resolve_errors_when_explicit_file_missing` | — |
| fatal error and exit if config.js contains unresolved env var  | 126 | pending | — | — | JS config files / env var resolution (TS-specific) |
| fatal error and exit if %s | 147 | ported | `file.rs` | `load_rejects_unsupported_or_missing_extension` | — |
| exports env variables to environment from processEnv object  | 161 | pending | — | — | JS config files / env var resolution (TS-specific) |
| does not export env variables to environment from processEnv object if key/value is invalid  | 184 | pending | — | — | JS config files / env var resolution (TS-specific) |
