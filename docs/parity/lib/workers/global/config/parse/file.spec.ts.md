...
|---|---|---|---|---|---|
| parses %s > %s | 27 | not-applicable | — | — | Loads JS/TS/CJS/MJS config files via Node.js dynamic import — no Rust equivalent for JS config loading |
| migrates | 56 | not-applicable | — | — | Requires JS config file migration via dynamic import |
| warns if config is invalid | 68 | not-applicable | — | — | Requires writing/reading a JS config file from temp dir with validation |
...
| fatal error and exit if error in parsing %s | 84 | not-applicable | — | — | Requires loading various JS/TS fixture config files via Node.js import with process.exit spy |
| fatal error and exit if custom config file does not exist | 112 | ported | `file.rs` | `resolve_errors_when_explicit_file_missing` | — |
| fatal error and exit if config.js contains unresolved env var | 126 | not-applicable | — | — | Requires JS config file with env var substitution via Node.js import |
| fatal error and exit if %s | 147 | ported | `file.rs` | `load_rejects_unsupported_or_missing_extension` | — |
| exports env variables to environment from processEnv object | 161 | not-applicable | — | — | Tests processEnv export from JS config — JS runtime infrastructure |
| does not export env variables to environment from processEnv object if key/value is invalid | 184 | not-applicable | — | — | Tests processEnv validation in JS config — JS runtime infrastructure |
