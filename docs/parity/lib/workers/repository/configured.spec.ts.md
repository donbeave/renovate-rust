# Renovate Test Detail

[Back to test map](../../../renovate-test-map.md)

## `lib/workers/repository/configured.spec.ts`

**Reference:** https://github.com/renovatebot/renovate/blob/main/lib/workers/repository/configured.spec.ts
**Total tests:** 3 | **Ported:** 3 | **Actionable:** 0 | **Status:** done

### `workers/repository/configured › checkIfConfigured()`

| Original test name | Line | Status | Rust file | Rust test name | Reason |
|---|---|---|---|---|---|
| returns | 16 | ported | `util.rs` | `test_check_if_configured_ok` | — |
| throws if disabled | 20 | ported | `util.rs` | `test_check_if_configured_disabled` | — |
| throws if unconfigured fork | 25 | ported | `util.rs` | `test_check_if_configured_fork` | — |

---

