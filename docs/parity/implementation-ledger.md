...
|-------|------------|--------------------------------|----------|-------|
| 0337  | 2026-04-29 | Output enhancements: `versioning` added to `DepReport` JSON output and wired from `RuleEffects`; `updateType` now set to `"replacement"` when `replacementName` is present (overrides semver classification); all 34+ files updated | Complete | See below. |
