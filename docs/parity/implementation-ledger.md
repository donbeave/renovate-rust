...
| 0363  | 2026-04-29 | `removedPresets` normalization + extends string-to-array: `normalize_preset()` applies full Renovate `removedPresets` map with chained resolution (`:unpublishSafe`→`npm:unpublishSafe`→`security:minimumReleaseAgeNpm`); `extends` + `schedule` accept bare strings via `deserialize_string_or_vec`; 5 new tests from migration.spec.ts; 1751 tests | Complete | See below. |
| 0362  | 2026-04-29 | Schedule string migration + string-or-array deserialization: `"every friday"` → `"on friday"` (migrate_schedule_string); `schedule`/`automergeSchedule` now accept bare strings via `deserialize_string_or_vec`; 3 new tests from migration.spec.ts; 1746 tests | Complete | See below. |
| 0361  | 2026-04-29 | Port all deprecated packageRule field migrations from `migration.spec.ts`: 13 new deprecated fields handled: `paths`→matchFileNames, `languages`→matchCategories, `baseBranchList`→matchBaseBranches, `managers`→matchManagers, `datasources`→matchDatasources, `depTypeList`→matchDepTypes, `packageNames`→matchPackageNames, `packagePatterns`→`/pat/`, `excludePackageNames`→`!name`, `excludePackagePatterns`→`!/pat/`, `excludeRepositories`→`!repo`, `sourceUrlPrefixes`→`url{/,}**`, `updateTypes`→matchUpdateTypes; 4 new tests; 1743 tests | Complete | See below. |
...
| 0349  | 2026-04-29 | `isBump` support: `UpdateType::Bump` variant added; `"bump"` now parsed in `matchUpdateTypes`; `DepContext.is_bump: bool` field added; `update_type_matches(ut, is_bump)` checks `Bump` variant when `is_bump` is true (mirrors Renovate's `UpdateTypesMatcher` pushing `'bump'` to checked set); 2 new tests ported from `index.spec.ts` | Complete | See below. |
| 0348  | 2026-04-29 | `string_match` parity fixes: (1) glob matching now case-insensitive (`GlobBuilder::case_insensitive(true)`) matching Renovate's `minimatch({nocase:true})`; (2) `literal_separator(true)` so bare `**` not at path boundary doesn't cross `/` (matches minimatch path-boundary rules); (3) new `any_match_regex_or_glob_list` function mirrors Renovate's `anyMatchRegexOrGlobList`; (4) 10 new tests ported from `string-match.spec.ts` + `dep-names.spec.ts`; 1708 tests total | Complete | See below. |
...
| 0342  | 2026-04-29 | More deprecated field migrations: `ignoreNodeModules: true` → adds `"node_modules/"` to `ignorePaths`; `enabledManagers: ["yarn"]` → `"npm"`, `"regex"` → `"custom.regex"` (matching Renovate's enabledManagers migration); 2 tests | Complete | See below. |
...
| 0334  | 2026-04-29 | `packages:*` preset namespace + `extends` inside packageRules: 18 packages presets (angularJs, apollographql, atlaskit, emberTemplateLint, eslint, gatsby, googleapis, jsTest, jsUnitTest, linters, mapbox, phpLinters, phpUnitTest, postcss, react, stylelint, test/unitTest, tslint, vite); `extends: Vec<String>` field added to RawPackageRule; `resolve_package_rule_extends()` merges matchPackageNames/matchDatasources/matchSourceUrls from presets into the rule; `has_name_constraint` updated; 4 tests | Complete | See below. |
| 0333  | 2026-04-29 | Output enhancements: `dependencyDashboardApproval`, `replacementName`, `replacementVersion` added to `DepReport` JSON output and wired from `RuleEffects` via `pipeline_utils`; all ~80 DepReport construction sites across 33 pipeline files updated | Complete | See below. |
| 0332  | 2026-04-29 | Missing boolean config flags + preset support: `respect_latest`, `pin_digests`, `dependency_dashboard`, `dependency_dashboard_approval`, `config_migration` fields on RepoConfig; `dependency_dashboard_approval` per-rule field on PackageRule/RuleEffects; presets: `:respectLatest`, `:dependencyDashboard`, `:disableDependencyDashboard`, `:dependencyDashboardApproval`, `:configMigration`, `:pinDigestsDisabled`, `:approveMajorUpdates`; 9 tests | Complete | See below. |
| 0331  | 2026-04-29 | `custom-managers:*` preset namespace (10 presets): `resolve_extends_custom_managers()` wired into `parse()`; `dockerfileVersions`, `makefileVersions`, `githubActionsVersions`, `gitlabPipelineVersions`, `azurePipelinesVersions`, `bitbucketPipelinesVersions`, `helmChartYamlAppVersions`, `mavenPropertyVersions`, `tfvarsVersions`, `tsconfigNodeVersions` (2 rules); fixed `r"..."` raw-string `"` bug with `r#"..."#`; 6 tests | Complete | See below. |
