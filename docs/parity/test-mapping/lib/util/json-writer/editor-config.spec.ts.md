# `lib/util/json-writer/editor-config.spec.ts`

[← `util/json-writer`](../../../_by-module/util/json-writer.md) · [all modules](../../../README.md)

**0/5 ported** (5 pending) · status: pending

| Line | Test | Status | Rust destination |
|--:|---|---|---|
| 31 | should handle empty .editorconfig file | pending | — |
| 40 | should handle global config from .editorconfig | pending | — |
| 51 | should return undefined in case of exception | pending | — |
| 59 | should not handle non json config from .editorconfig | pending | — |
| 70 | should handle json config from .editorconfig | pending | — |

