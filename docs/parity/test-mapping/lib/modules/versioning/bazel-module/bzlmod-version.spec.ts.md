# `lib/modules/versioning/bazel-module/bzlmod-version.spec.ts`

[← `versioning/bazel-module`](../../../../_by-module/versioning/bazel-module.md) · [all modules](../../../../README.md)

**14/18 ported** (4 pending) · status: partial

| Line | Test | Status | Rust destination |
|--:|---|---|---|
| 5 | when all digits | ported | `crates/renovate-core/src/versioning/bazel_module/bzlmod_version.rs:221` |
| 12 | when not all digits | ported | `crates/renovate-core/src/versioning/bazel_module/bzlmod_version.rs:230` |
| 19 | _(it.each / template — verify manually)_ | ? | — |
| 32 | _(it.each / template — verify manually)_ | ? | — |
| 49 | _(it.each / template — verify manually)_ | ? | — |
| 59 | _(it.each / template — verify manually)_ | ? | — |
| 68 | _(it.each / template — verify manually)_ | ? | — |
| 78 | _(it.each / template — verify manually)_ | ? | — |
| 87 | _(it.each / template — verify manually)_ | ? | — |
| 96 | _(it.each / template — verify manually)_ | ? | — |
| 107 | _(it.each / template — verify manually)_ | ? | — |
| 125 | _(it.each / template — verify manually)_ | ? | — |
| 137 | _(it.each / template — verify manually)_ | ? | — |
| 153 | _(it.each / template — verify manually)_ | ? | — |
| 168 | _(it.each / template — verify manually)_ | ? | — |
| 188 | _(it.each / template — verify manually)_ | ? | — |
| 208 | _(it.each / template — verify manually)_ | ? | — |
| 221 | _(it.each / template — verify manually)_ | ? | — |

