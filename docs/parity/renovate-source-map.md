...
| `lib/config/presets/internal/helpers.preset.ts` | `crates/renovate-core/src/repo_config.rs` | full | All scanning-relevant presets implemented: `pinGitHubActionDigests`, `pinGitHubActionDigestsToSemver`, `disableTypesNodeMajor`, `followTypescriptNext/Rc`, `*DigestChangelogs`; `goXPackages*` uses `prBodyDefinitions` (out-of-scope PR formatting) |
...
| `lib/config/presets/internal/packages.preset.ts` | `crates/renovate-core/src/repo_config.rs` | full | 18 `packages:*` presets fully exposed via `extends` inside packageRules; matchers merged into rule |
