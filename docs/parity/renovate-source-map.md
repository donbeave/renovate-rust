...
| `lib/config/global.ts` | `crates/renovate-core/src/config.rs` | partial | Global option list and config types partially ported, including mode, printConfig, autodiscover filters, prCommitsPerRunLimit, gitPrivateKeyPassphrase, onboarding/config-file, optimizeForDisabled, gitUrl, object-valued, and scalar self-hosted global option retention |
