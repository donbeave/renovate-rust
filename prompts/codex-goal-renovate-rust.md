...
```text
Use prompts/codex-goal-renovate-rust.md as the active Codex goal file. Prepare the goal before implementation in Codex goal format: read this file's Objective, Definition Of Done, Operating Rules, and Progress Loop; inspect the repository state and recent commits; inspect parity docs; and create the current working plan from those sections. Use prompts/claude-loop-renovate-rust.md only as the implementation playbook for each iteration: repository rules, Renovate reference workflow, parity tracking, Rust standards, commit policy, push policy, and verification policy. Then execute this file's Progress Loop repeatedly until renovate-rust provides a production-quality Rust `renovate` binary that is a Renovate-compatible drop-in replacement for common self-hosted CLI workflows, including compatible CLI flags, environment variables, config discovery and semantics, exit codes, dependency extraction, datasource/versioning decisions, update planning, output modes, and parity tracking. Each turn must choose the next highest-value compatibility gap, implement a coherent slice, update parity docs, commit it, push it to main, and immediately continue with the next slice. Do not treat prompts/claude-loop-renovate-rust.md as the completion condition. Do not stop after one implementation slice, after partial parity progress, or merely because `git status --short` is clean. The goal is complete only when this goal file's Definition Of Done is satisfied. Do not run verification commands unless the operator explicitly asks.
```
