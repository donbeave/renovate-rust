...
```text
/goal Use @renovate-rust/prompts/codex-goal-renovate-rust.md as the active Codex goal file. Prepare the goal before implementation in Codex goal format: read that file's Objective, Definition Of Done, Operating Rules, and Progress Loop; inspect the repository state and recent commits; inspect parity docs; and create the current working plan from those sections. Use @renovate-rust/prompts/claude-loop-renovate-rust.md only as the implementation playbook for each iteration: repository rules, Renovate reference workflow, parity tracking, Rust standards, commit policy, push policy, and verification policy. Then execute the Codex goal file's Progress Loop repeatedly until renovate-rust provides a production-quality Rust `renovate` binary that is a Renovate-compatible drop-in replacement for common self-hosted CLI workflows, including compatible CLI flags, environment variables, config discovery and semantics, exit codes, dependency extraction, datasource/versioning decisions, update planning, output modes, and parity tracking. Each turn must choose the next highest-value compatibility gap, implement a coherent slice, update parity docs, commit it, push it to main, and immediately continue with the next slice. Do not treat the loop file as the completion condition. Do not stop after one implementation slice, after partial parity progress, or merely because `git status --short` is clean. The goal is complete only when the Definition Of Done in the Codex goal file is satisfied. Do not run verification commands unless the operator explicitly asks; if blocked, document the blocker, commit and push any coherent progress, then continue with another local/offline slice.
```
...
```text
Use prompts/codex-goal-renovate-rust.md as the active Codex goal file. Prepare the goal before implementation in Codex goal format: read the Objective, Definition Of Done, Operating Rules, and Progress Loop; inspect the repository state and recent commits; inspect parity docs; and create the current working plan from those sections. Use prompts/claude-loop-renovate-rust.md only as the implementation playbook for each iteration. Execute the Codex goal file's Progress Loop repeatedly until renovate-rust provides a production-quality Rust `renovate` binary that is a Renovate-compatible drop-in replacement for common self-hosted CLI workflows. Keep choosing the next highest-value compatibility gap, updating parity docs, committing each coherent slice, pushing every commit to main, and immediately continuing until the Definition Of Done in the goal file is satisfied. Do not treat the loop file, one slice, partial parity progress, or `git status --short` as the finish line. Do not run verification commands unless the operator explicitly asks.
```
